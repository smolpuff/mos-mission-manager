...
function shouldRequestOwnedCooldownFollowup(rentalResult = {}) {
  return Boolean(
    rentalResult?.stopReason === "no_due_candidate" ||
      rentalResult?.reason === "no_due_candidate" ||
      rentalResult?.reason === "snapshot_batch_complete" ||
      (rentalResult?.capped === true && rentalResult?.hasMoreDue === false),
  );
}
...
function createChecksService(ctx, logger, mcp, services = {}) {
  const { logWithTimestamp, logDebug, redrawHeaderAndLog, formatTaggedLog } =
    logger;
  const { signer = null, rentals = null } = services;
  ...
  let rentalDemandMissionKeys = new Set();
  ...
  async function prepareUnlockSlot4({ reason = "ui_manual_unlock" } = {}) {
    ...
    const missionsResult =
      cachedMissionsResult ||
      (await mcp.getUserMissions({
        forceFresh: true,
        reason,
      }));
    logDebug("assign", "slot_unlock_mission_state", {
      reason,
      source: cachedMissionsResult ? "existing_ui_snapshot" : "mcp_refresh",
      ageMs: cachedMissionsResult
        ? Math.max(0, Date.now() - Number(ctx.lastUserMissionsFetchedAt || 0))
        : 0,
    });
    if (ctx.signerMode === "manual") {
      const summary = extractSlotUnlockSummary(missionsResult);
    ...
    if (result?.unlocked) {
      ctx.slotUnlockSummary = {
        ...(ctx.slotUnlockSummary && typeof ctx.slotUnlockSummary === "object"
          ? ctx.slotUnlockSummary
          : {}),
        canUnlockMore: false,
        nextUnlockSlot: null,
        availableSlots: Math.max(
          TARGET_UNLOCK_SLOT,
          Number(ctx.slotUnlockSummary?.availableSlots || 0),
        ),
      };
      if (ctx.guiBridge?.emitNow) ctx.guiBridge.emitNow();

      let postUnlockMissions = null;
      for (let attempt = 1; attempt <= 2; attempt += 1) {
        try {
          postUnlockMissions = await mcp.getUserMissions({
            forceFresh: true,
            reason: "post_slot4_unlock_refresh",
          });
          break;
        } catch (error) {
          if (error?.rateLimited !== true || attempt >= 2) throw error;
          const waitMs = Math.max(
            1000,
            Number(error.retryAfterSeconds || 60) * 1000 + 250,
          );
          logWithTimestamp(
            `[UNLOCK] ⏳ Post-unlock mission refresh queued for ${Math.ceil(waitMs / 1000)}s; not retrying early.`,
          );
          await new Promise((resolve) => setTimeout(resolve, waitMs));
        }
      }
      await refreshMissionHeaderStats({
        missionsResult: postUnlockMissions,
        refreshNftCount: false,
        hydrateAssignedMetadata: false,
      });
      const assignResult = await autoAssignConfiguredMissions({
        reason: "post_slot4_unlock",
        missionsResult: postUnlockMissions,
      });
      logDebug("assign", "slot_unlock_followup_complete", {
        reason,
        missions: normalizeMissionList(postUnlockMissions).length,
  ...
  async function applyMissionSelection({
    ...
    const hasDirectSelectedMission =
      isUsableIdValue(selectedMissionId) &&
      String(selectedMissionName || "").trim().length > 0;
    if (
      !hasDirectSelectedMission &&
    ...
    let missionsResult =
      ...
      null;
    let missions = normalizeMissionList(missionsResult || {});
    if (missions.length === 0 && Array.isArray(ctx.guiMissionSlots)) {
      missions = ctx.guiMissionSlots
        .filter((entry) => entry && entry.missionName)
        .map((entry) => ({
          slot: entry.slot,
          assigned_mission_id:
            entry.assignedMissionId || entry.missionId || null,
          name: entry.missionName,
          current_level: entry.missionLevel ?? null,
          assigned_nft: entry.assignedNftAccount || null,
        }));
    }
    const summary =
      normalizeSlotUnlockSummary(missionsResult) || ctx.slotUnlockSummary;
    ...
    const selected =
      ...
      (hasDirectSelectedMission
        ? {
            id: String(selectedMissionId).trim(),
            mission_id: String(selectedMissionId).trim(),
            name: String(selectedMissionName).trim(),
          }
        : null);
    if (!selected) {
    ...
    const currentMission =
      missions.find((mission) => Number(mission?.slot) === slotNumber) ||
      (isUsableIdValue(suppliedAssignedMissionId)
        ? {
            slot: slotNumber,
            assigned_mission_id: String(suppliedAssignedMissionId).trim(),
            name: String(suppliedCurrentMissionName || "").trim(),
          }
        : null);
    ...
    if (currentAssignedMissionId) {
      ...
      if (mutationMissionResult && publishMissionState) {
        await refreshMissionHeaderStats({
    ...
    const assignmentMissionResult = assignResult?.missionResult || missionsResult;
    if (assignmentMissionResult && publishMissionState) {
      await refreshMissionHeaderStats({
        missionsResult: assignmentMissionResult,
        refreshNftCount: false,
        hydrateAssignedMetadata: false,
      });
    }
  ...
  function rentalCoordinatorEnabled() {
    return Boolean(rentals && ctx.config?.enableRentals === true);
  }

  function getMissionsNeedingRental() {
    if (!rentalCoordinatorEnabled()) return [];
    const result = ctx.lastUserMissionsResult;
    if (!(result && typeof result === "object")) return [];
    return buildAssignCandidates(
      normalizeMissionList(result),
      resolveConfiguredTargets(),
      extractSlotUnlockSummary(result),
      "rental_coordinator",
    )
      .filter(
        (mission) => !missionEligibleForAutoModeThresholdFallbackReset(mission),
      )
      .filter((mission) =>
        rentalDemandMissionKeys.has(assignedMissionId(mission)),
      );
  }

  function missionStillNeedsRental(missionKey, fallbackMission = null) {
    const wanted = String(missionKey || "").trim();
    const missions = getMissionsNeedingRental();
    return missions.some((mission) => {
      const id = assignedMissionId(mission);
      const slot = Number(mission?.slot);
      return (
        (wanted && id === wanted) ||
        (wanted && Number.isFinite(slot) && String(slot) === wanted) ||
        (fallbackMission && mission === fallbackMission)
      );
    });
  }

  function stopRentalCoordinator(reason = "watch_stopped") {
    rentalDemandMissionKeys = new Set();
    if (rentals && typeof rentals.setDemand === "function") {
      rentals.setDemand(false, { reason });
    }
  }

  async function assignCachedRentalCandidate({
    mission,
    missionKey,
    candidate,
    boundaryRetry = false,
    assignmentBudgetReservation = null,
    assignmentBudgetToken = null,
  } = {}) {
    if (!missionStillNeedsRental(missionKey, mission)) {
      if (assignmentBudgetToken) {
        mcp.releaseToolWindowReservation(assignmentBudgetToken);
      }
      return { success: false, kind: "mission_filled", message: "Mission no longer needs an NFT." };
    }
    const missionId = assignedMissionId(mission) || String(missionKey || "").trim();
    const listingId = String(candidate?.listingId || "").trim();
    const nftAccount = String(candidate?.nftAccount || candidate?.account || "").trim();
    if (!missionId || !listingId || !nftAccount) {
      if (assignmentBudgetToken) {
        mcp.releaseToolWindowReservation(assignmentBudgetToken);
      }
    ...
    nftAssignmentLastUsedAt.set(nftAccount, Date.now());
    ensureNftAssignmentUsageLoaded();
    nftAssignmentUsage.set(
      nftAccount,
      (nftAssignmentUsage.get(nftAccount) || 0) + 1,
    );
  ...
  async function reconcileAmbiguousRentalAssignment({
    ...
    try {
      const result = await mcp.getUserMissions({
        forceFresh: true,
        reason: "rental_assignment_reconcile",
      });
      const current = findMissionByAssignedMissionId(
        normalizeMissionList(result),
        assignedMissionId(mission) || missionKey,
      );
      const assignedAccount = missionAssignedNftAccount(current);
      if (assignedAccount && assignedAccount === candidate?.nftAccount) {
        return { assigned: true, missionState: result };
      }
      if (!current || missionHasAssignedNftForAssign(current, "rental_reconcile")) {
        return { assigned: false, safeToRelease: false, missionState: result };
      }
      return { assigned: false, safeToRelease: true, missionState: result };
    } catch (error) {
      logDebug("rental", "ambiguous_reconcile_failed", {
        missionKey,
        listingId: candidate?.listingId || null,
        error: error.message,
      });
      return { assigned: false, safeToRelease: false, error: error.message };
    }
  }
  ...
  async function autoAssignConfiguredMissions(options = {}) {
    ...
    let result;
    try {
      result = await autoAssignPromise;
    } finally {
      autoAssignPromise = null;
    }
    if (
      result?.ownedCooldownFollowupRequested === true &&
      options.skipRentalCoordinator !== true
    ) {
      const followup = await autoAssignConfiguredMissions({
        reason: `${options.reason || "periodic"}_owned_cooldown_fallback`,
  ...
  async function runAutoAssignConfiguredMissions({
    ...
    if (resolved.targetIds.size === 0 && resolved.targetNames.size === 0) {
      rentalDemandMissionKeys = new Set();
      if (rentals) rentals.setDemand(false, { reason: "no_configured_targets" });
    ...
    try {
      ...
      if (candidates.length === 0) {
        rentalDemandMissionKeys = new Set();
        if (rentals) rentals.setDemand(false, { reason: "no_open_missions" });
        if (reason === "manual") {
          logWithTimestamp(
      ...
      let assigned = 0;
      let rentalDemandDetected = false;
      const rentalDemandKeysThisPass = new Set();
      ...
      const assignmentPassNftScan =
        candidates.length > 1
          ...
          : { ...nftAssignmentScan };
      // Take one coherent inventory snapshot for every open mission before
      // the first assignment mutates mission/NFT state. Loading slot 2 only
      // after slot 1 was assigned made a single logical pass depend on a
      // second, post-mutation inventory read and commonly deferred the rest
      // of the slots until the next watcher cycle.
      ...
      const inventorySnapshotResults = await Promise.all(
        candidates.map(async (mission) => {
          const missionId = assignedMissionId(mission);
          if (!missionId) {
            return { missionId: "", mission, ok: false, nfts: [] };
          }
      ...
      for (const snapshot of inventorySnapshotResults) {
        ...
        if (snapshot.ok) {
          ...
          logDebug("assign", "eligible_nfts_loaded", {
            ...
            nftPageOffset: snapshot.nftPageOffset,
            nftScanCycle: assignmentPassNftScan.cycle,
            nftCount: snapshot.nfts.length,
            eligibleCount: snapshot.nfts.filter(nftIsAvailable).length,
            cooldownCount: snapshot.nfts.filter(
              (nft) => !nftIsAvailable(nft),
            ).length,
            assignedElsewhereCount: snapshot.nfts
              .map((nft) => nftAccountId(nft))
              .filter(
                (account) => account && alreadyAssignedNftAccounts.has(account),
              ).length,
          ...
          logDebug("assign", "nft_list_failed", {
            missionId: snapshot.missionId,
            name: missionName(snapshot.mission),
      ...
      for (const mission of candidates) {
        ...
        if (selectedReadyOwnedCandidates.length > 0) {
          ...
          rentalDemandDetected = true;
          rentalDemandKeysThisPass.add(id);
          logWithTimestamp(
            `[RENTAL] 📋 ${name}: no ready owned NFT; queued for cached rental assignment.`,
          );
          continue;
        } else if (
          ...
          } else if (ownedCooldownCandidates.length > 0) {
            logWithTimestamp(
        ...
        for (let index = 0; index < assignmentOptions.length; index += 1) {
          ...
          const ownedNftWasAvailable = nftIsAvailable(nft);
          ...
          try {
            ...
            const assignResult = await mcp.mcpToolCall(
              "assign_nft_to_mission",
              { assignedMissionId: id, nftAccount: account },
            );
            logWithTimestamp(
            ...
            if (
              ...
              const usedReset = option.source === "owned_cooldown";
              const normalizedSource = "owned";
              const nftLevel = Number.isFinite(Number(nft?.level))
                ? Number(nft.level)
                : null;
      ...
      if (rentals && skipRentalCoordinator !== true) {
        ...
        const rentalsNeeded =
          rentalCoordinatorAvailable &&
          (rentalDemandDetected || rentalMissions.length > 0);
        rentals.setDemand(rentalsNeeded, { reason });
        if (rentalsNeeded) {
          await rentals.requestSearch({ reason, rentalsNeeded: true });
          const rentalResult = await rentals.runAssignmentWorker({
            reason,
            missions: rentalMissions,
          });
          assigned += Number(rentalResult?.successes || 0);
          logDebug("rental", "coordinator_pass_complete", {
            reason,
            attemptedDistinct: Number(rentalResult?.attemptedDistinct || 0),
            dispatches: Number(rentalResult?.dispatches || 0),
            successes: Number(rentalResult?.successes || 0),
            stopReason: rentalResult?.stopReason || rentalResult?.reason || null,
          });
          rentals.setDemand(getMissionsNeedingRental().length > 0, {
            reason: `${reason}_post_assignment`,
          });
          ownedCooldownFollowupRequested =
            autoNftCooldownResetEnabled() &&
            getMissionsNeedingRental().length > 0 &&
            shouldRequestOwnedCooldownFollowup(rentalResult);
        }
      }
      ...
      rentalDemandMissionKeys = new Set();
      if (rentals) rentals.setDemand(false, { reason: "assignment_check_failed" });
  ...
  async function claimClaimableMissions({
    ...
    try {
      ...
      if (candidates.length === 0) {
        ...
        if (ctx.guiBridge?.emitNow) ctx.guiBridge.emitNow();
        return { ok: true, claimed: 0, claims: [], missionResult: result };
      }
      if (ctx.guiBridge?.sendEvent) {
        ctx.guiBridge.sendEvent("claiming", {
          state: "start",
      ...
      for (const mission of candidates) {
        ...
        try {
          ...
          if (!claimResponseHasMissions) {
            ...
            // The mutation itself succeeded, so the claim must still be
            // counted and emitted even if this server version omits its
            // replacement mission rows. Do not expose the pre-claim snapshot
            // as authoritative; the shared lifecycle will preserve the
            // optimistic UI and wait for a permitted fresh read.
            authoritativeMissionResult = null;
          }
          ...
          if (claimMutationStateMissing) break;
        } catch (error) {
          if (
            claimsPaused() ||
  ...
  async function runWalletSummaryCheck({ includeWalletSummary = true } = {}) {
    try {
      const json = await mcp.mcpToolCall(
        "get_wallet_summary",
        {},
        { reason: "wallet_summary_check" },
      );
  ...
  async function refreshMissionHeaderStats({
    ...
    const refreshSequence = preserveMissionState
      ? null
      : ++missionHeaderRefreshSequence;
    try {
      const result =
        missionsResult ||
        (await mcp.getUserMissions({
      ...
      const snapshotRevision =
        typeof mcp.getMissionSnapshotRevision === "function"
          ? mcp.getMissionSnapshotRevision(result)
          : null;
      const currentSnapshotRevision = () =>
        typeof mcp.getCurrentMissionSnapshotRevision === "function"
          ? Number(mcp.getCurrentMissionSnapshotRevision() || 0)
          : 0;
      const staleSnapshotReason = () => {
        const currentRevision = currentSnapshotRevision();
        if (currentRevision <= 0) return null;
        if (Number.isFinite(Number(snapshotRevision))) {
          return Number(snapshotRevision) < currentRevision
            ? "older_revision"
            : null;
        }
        // Once the production MCP client has accepted mission state, only a
        // registered accepted snapshot may replace cards. This closes the
        // path where a captured pre-mutation object starts a refresh later and
        // incorrectly wins merely because its async refresh sequence is newer.
        return result !== ctx.lastUserMissionsResult
          ? "unregistered_snapshot"
          : null;
      };
      const rejectStaleSnapshot = (phase) => {
        const staleReason = staleSnapshotReason();
        if (!staleReason) return null;
        logDebug("check", "mission_header_snapshot_rejected", {
          phase,
          staleReason,
          snapshotRevision,
        ...
        return {
          ok: false,
          superseded: true,
          staleMissionSnapshot: true,
          staleReason,
        };
      };
      const rejectedBeforeWork = rejectStaleSnapshot("before_work");
      if (rejectedBeforeWork) return rejectedBeforeWork;
      const missions = normalizeMissionList(result);
      if (!preserveMissionState) {
        updateMissionLookupCache(result, missions);
      }
      if (!preserveMissionState && syncTargetsFromAssigned) {
      ...
      if (refreshNftCount) {
        try {
          ...
          nftRefreshDeferred = {
            rateLimited: error?.rateLimited === true,
            retryAfterSeconds:
              Number(error?.retryAfterSeconds || 0) || null,
            error: error.message,
          };
        }
      }
      ...
      if (
        ...
        refreshSequence !== missionHeaderRefreshSequence
      ) {
        logDebug("check", "mission_header_refresh_superseded", {
          refreshSequence,
          latestSequence: missionHeaderRefreshSequence,
          syncReason: syncReason || null,
        });
        return { ok: false, superseded: true, nftRefreshDeferred };
      }

      const rejectedBeforePublish = rejectStaleSnapshot("before_publish");
      if (rejectedBeforePublish) {
        return { ...rejectedBeforePublish, nftRefreshDeferred };
      }
      ...
      if (preserveMissionState) {
        ...
        return { ok: true, nftRefreshDeferred, preservedMissionState: true };
      }
  ...
  async function runInitialChecks() {
    ...
    if (!walletIdentity.ok) {
      if (walletIdentity.rateLimited && mcp.bearerToken()) {
        ctx.isAuthenticated = true;
        logWithTimestamp(
          `[CHECK] ⏳ Wallet data deferred by MCP cooldown (${walletIdentity.retryAfterSeconds || "?"}s); saved authenticated session retained.`,
        );
      } else {
  ...
  return {
    ...
    assignCachedRentalCandidate,
    reconcileAmbiguousRentalAssignment,
    logSelectedWatchTargetsAtStartup,
    autoAssignConfiguredMissions,
    stopRentalCoordinator,
    prepareUnlockSlot4,
