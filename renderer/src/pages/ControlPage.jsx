...
function ControlView() {
  ...
  const [
    ...
    setMissionResetPerSlotEnabledBySlot,
  ] = useState(
    normalizeSlotBooleanMap(status.missionResetPerSlotEnabledBySlot),
  );
  const [missionResetPerSlotLevelBySlot, setMissionResetPerSlotLevelBySlot] =
    useState(
  ...
  }, [status.missionResetPerSlotModeEnabled]);

  useEffect(() => {
    setMissionResetPerSlotEnabledBySlot((current) => ({
      ...current,
      ...normalizeSlotBooleanMap(status.missionResetPerSlotEnabledBySlot),
    }));
  }, [status.missionResetPerSlotEnabledBySlot]);

  useEffect(() => {
    setMissionResetPerSlotLevelBySlot((current) => ({
      ...current,
      ...normalizeSlotLevelMap(
        status.missionResetPerSlotLevelBySlot,
        status.currentMissionResetLevel ?? 10,
  ...
  useEffect(() => {
    ...
    bridge.getConfig().then((response) => {
      ...
      setMissionResetPerSlotEnabledBySlot(
        normalizeSlotBooleanMap(config.missionResetPerSlotEnabledBySlot),
      );
  ...
  const setPerSlotMissionResetLevel = async (slot, value) => {
    const key = String(slot);
    const raw = String(value ?? "").trim();
    ...
    setMissionResetPerSlotLevelBySlot(nextLevelBySlot);
    const nextLevel = Number(raw);
    if (!Number.isFinite(nextLevel) || nextLevel <= 0) return;
    await applyConfigPatch({
      missionResetPerSlotLevelBySlot: {
        ...nextLevelBySlot,
        [key]: Math.floor(nextLevel),
      },
    });
  };
  ...
  const confirmUnlockSlot4 = async () => {
    ...
    try {
      ...
      if (
        prepared?.reason === "no_more_to_unlock" ||
        prepared?.reason === "not_unlockable"
      ) {
  ...
  const seedMissionSelectionState = (missions = [], targetMissions = []) => {
    const sourceMissions =
      Array.isArray(missions) && missions.length ? missions : slots;
  ...
  const openMissionPicker = async (slot) => {
    ...
    const currentName = String(
      onboardingSlotSelections[slotNumber] ||
        slots.find((entry) => Number(entry?.slot) === slotNumber)
          ?.missionName ||
        "",
    ).trim();
        ...
        setOnboardingOwnedCollections(
          new Set(
            (Array.isArray(accountResponse.ownedCollections)
              ...
              .map((entry) =>
                normalizeCollectionKey(collectionNameFromEntry(entry)),
              )
              .filter(Boolean),
          ),
        );
  ...
  const applyMissionPickerSelection = async () => {
    ...
    if (!bridge?.applyMissionSelection) {
      setMissionPickerError(
        "Mission selection is not available in this build.",
      );
      return;
    }
    if (
  ...
  return (
    <main className="shell flex flex-col">
      <WindowChrome title="missions-v3-mcp" styling="min-w-full" />

      <div
        className="flex min-h-0 flex-1 gap-0 w-full overflow-hidden"
        id="main-window"
      >
        <NavMain
                ...
                <div className="flex items-center justify-between gap-3 border-t border-white/10 pt-3">
                  <div className="min-w-0 text-xs text-slate-400">
                    {missionPickerPendingName && missionPickerHasActualChange
                      ...
                      : missionPickerPendingName
                        ? `Current: ${missionPickerPendingName}`
                        : "Select a mission before confirming."}
                  </div>
                  <div className="flex items-center gap-2">
                    <button
