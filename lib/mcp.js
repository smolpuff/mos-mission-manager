...
function parseSseJson(text, fallback = null) {
  ...
  return parseJson(dataParts.join("\n"), fallback);
}
...
function extractToolResultPayload(callJson) {
  if (!callJson || typeof callJson !== "object") return {};
...
function throwIfToolResultError(payload, toolName) {
  ...
  const text = Array.isArray(payload.content)
    ? payload.content
        ...
        .join("\n")
    : "";
  throw new Error(text || `MCP tool ${toolName} returned an error result`);
}
...
function tokenExpiresSoon(record, skewMs = 90000) {
  ...
  if (!isNonEmptyString(exp)) return false;
...
async function toolsList({
  ...
  return extractToolResultPayload(res.json);
}
...
async function callTool(name, args = {}, opts = {}) {
  ...
  const explicitToken = opts.token || process.env.PBP_BEARER_TOKEN || null;
  const refreshable = !explicitToken;

  const ensureUsableToken = async ({
    allowRefreshOnExpiry = true,
    allowRefreshOnAuthError = true,
  } = {}) => {
    let saved = refreshable ? readTokenFile(tokenFile) : null;
    let token = getAccessToken({ token: opts.token, tokenFile });
    if (!token)
      throw new Error(
    ...
    if (
      refreshable &&
      allowRefreshOnExpiry &&
      saved?.refresh_token &&
      tokenExpiresSoon(saved)
    ) {
      await refreshAccessToken({ url, tokenFile, timeoutMs });
      saved = readTokenFile(tokenFile);
      token = getAccessToken({ token: opts.token, tokenFile });
      if (!token) {
    ...
    const run = async () => {
      ...
      const result = extractToolResultPayload(res.json);
      throwIfToolResultError(result, name);
      return result;
    };
    ...
    try {
      ...
      if (
        !refreshable ||
        !allowRefreshOnAuthError ||
        !saved?.refresh_token ||
        !isAuthHttpError(error?.message)
      ) {
        throw error;
      }
...
async function fetchJsonMetadata(url, timeoutMs = DEFAULT_TIMEOUT_MS) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), timeoutMs);
  try {
    ...
    if (!res.ok) return null;
    return parseJson(await res.text(), null);
  } catch {
    return null;
  } finally {
    clearTimeout(timeout);
...
async function discoverOAuthMetadata({
  ...
  const protectedResourceUrls = [
    ...
    new URL("/.well-known/oauth-protected-resource", parsedUrl.origin).toString(),
  ];

  let protectedResource = null;
  for (const metadataUrl of [...new Set(protectedResourceUrls)]) {
    protectedResource = await fetchJsonMetadata(metadataUrl, timeoutMs);
  ...
  const authorizationServers = Array.isArray(
    ...
    ? protectedResource.authorization_servers
    : [];
  const issuer = isNonEmptyString(authorizationServers[0])
    ? authorizationServers[0]
    : parsedUrl.origin;
  const issuerUrl = new URL(issuer);
  const issuerPath = issuerUrl.pathname.replace(/\/+$/, "");
  const authorizationServerUrls = [
    new URL(
  ...
  for (const metadataUrl of [...new Set(authorizationServerUrls)]) {
    const metadata = await fetchJsonMetadata(metadataUrl, timeoutMs);
    if (metadata) return metadata;
  }

  // Compatibility fallback for older MCP servers that exposed auth metadata
  // from a GET request to the MCP transport endpoint.
  try {
    const discovered = await discover({ url: baseUrl, timeoutMs });
...
async function login({
  ...
  const auth = await discoverOAuthMetadata({ url, timeoutMs });
  const authorizationEndpoint =
    resolveEndpoint(
      auth.authorization_endpoint || auth.authorizationEndpoint,
      baseUrl,
    ) ||
    new URL("/oauth/authorize", baseOrigin).toString();
  const tokenEndpoint =
    resolveEndpoint(auth.token_endpoint || auth.tokenEndpoint, baseUrl) ||
    new URL("/oauth/token", baseOrigin).toString();
  let registrationEndpoint =
    resolveEndpoint(
      auth.registration_endpoint || auth.registrationEndpoint,
      baseUrl,
    ) || null;
  if (!registrationEndpoint && isNonEmptyString(auth?.issuer)) {
    registrationEndpoint = await discoverRegistrationEndpoint(
      auth.issuer,
      timeoutMs,
    );
  }
  if (!registrationEndpoint && tokenEndpoint.endsWith("/token")) {
    registrationEndpoint =
      tokenEndpoint.slice(0, -"/token".length) + "/register";
  }
...
<html>
  <head>
    ...
    <style>
      ...
      .ok { font-weight: 600; font-size: 24px;}
      .muted { margin-top: 8px; font-size: 18px;}
      .love {font-size: 18px; margin-top: 8px;}
    </style>
  </head>
  <body>
    <div class="ok">Authentication complete.</div>
    <div class="muted">This window will close automatically.</div>
    <div class="love">❤️ korea</div>
    <script>
...
async function refreshAccessToken({
  ...
  const tokenEndpoint =
    resolveEndpoint(auth.token_endpoint || auth.tokenEndpoint, baseUrl) ||
    new URL("/oauth/token", baseOrigin).toString();
  const clientId = isNonEmptyString(saved.client_id)
    ? saved.client_id
    : DEFAULT_CLIENT_ID;
...
async function audit({
  ...
  const report = {
    ...
    wallet_summary: { ok: false, error: null },
  ...
  try {
    const summary = await callTool("get_wallet_summary", {}, {
      url,
      timeoutMs,
      tokenFile,
    });
    report.wallet_summary.ok = true;
    report.wallet_summary.result = summary;
  } catch (error) {
    report.wallet_summary.error = String(error.message || error);
  }
