...
async function openBrowser(url) {
  const compactSize = "100,100";
  const compactArg = "--window-size=100,100";
  const escapedUrl = String(url).replace(/\\/g, "\\\\").replace(/"/g, '\\"');
  const macChromeScript = [
    `tell application "Google Chrome"`,
    `activate`,
    `set w to make new window`,
    `set URL of active tab of w to "${escapedUrl}"`,
    `set bounds of w to {20, 20, 260, 360}`,
    `end tell`,
  ].join("\n");
  const macEdgeScript = [
  ...
  const candidates =
    process.platform === "darwin"
      ...
      : process.platform === "win32"
        ...
        : [
            ["google-chrome", ["--new-window", `--app=${url}`, compactArg]],
            ["chromium-browser", ["--new-window", `--app=${url}`, compactArg]],
            ["microsoft-edge", ["--new-window", `--app=${url}`, compactArg]],
            ["firefox", ["--new-window", "--width", "100", "--height", "100", url]],
            ["xdg-open", [url]],
            ["gio", ["open", url]],
          ];
